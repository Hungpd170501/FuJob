...
public class DisputeDAO {
    ...
    private static final String GET_LIST_HR_DISPUTE = "SELECT d.disputeID,j.userID as hrID,ja.resumeID,r.userID as studentID,j.disputeStatus, d.title, d.message, d.jobApplicationID, d.userID as creatorID, d.createdDate, d.lastModifiedDate "
            + "FROM (((tblDisputes d left join tblJobApplications ja on ja.jobApplicationID = d.jobApplicationID) left join  "
            + "tblJobs j on j.jobID = ja.jobID) "
            + "left join tblResumes as r on r.resumeID = ja.resumeID ) "
