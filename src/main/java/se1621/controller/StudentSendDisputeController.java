 ...
 * @author HNGB
 */
@WebServlet(name = "StudentSendDisputeController", urlPatterns = {"/StudentSendDisputeController"})
public class StudentSendDisputeController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean checkDup = disputeDAO.checkDuplicateDispute(studentID, jobApplicationID);//ktra da gui dispute truoc do chua
            if (checkDup) { //neu co thi xoa tao lai
                boolean checkDelete = disputeDAO.deleteDispute(studentID, jobApplicationID);
                if (checkDelete) {
                    ...
                    if (checkCreateDispute) {
                        ...
                        User student = usDAO.getUser(studentID);
                        boolean checkUpdateStatus = jobDAO.updateDisputeStatus(1, jobID);
                        if (checkUpdateStatus) {
                            EmailServiceImpl emailServiceIml = new EmailServiceImpl();
                            new Thread(() -> emailServiceIml.sendEmail(getServletContext(), hr, student.getFullName(), titleDispute, messageDispute, job.getJobTitle())).start();
